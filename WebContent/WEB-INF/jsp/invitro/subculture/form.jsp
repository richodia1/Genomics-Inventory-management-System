...
<title>In-Vitro germplasm subculturing: <s:property value="description.transactionType" /></title>
</head>
<body>
...
<s:if test="readonly">
	<s:form namespace="/update" action="invitro/subculture!saveDescription" method="POST">
		<table class="inputform">
			...
			<tr>
				<td />
				<td>
					<s:submit value="Export to XLS" action="invitro/subculture!exportXLS" />
					<authz:authorize ifAnyGranted="ROLE_ADMIN,ROLE_SUPERVISOR">
					<s:if test="description.status == 1">
						<s:submit value="Revert changes" action="invitro/subculture!rollback"
							onclick="javascript: return confirm('Are you sure you want to REVERT the changes?');" />
					</s:if>
				</authz:authorize> <s:if test="description.transactionSubtype == 'SUBCULTURING'">
					<s:submit value="Register subcultured lots" action="invitro/subcultured!input" />
				</s:if> <s:submit name="redirect-action:index" value="To list" /></td>
			</tr>
		</table>
...
<s:else>
	<s:form namespace="/update" action="invitro/subculture!saveDescription" method="POST">
		<table class="inputform">
			...
			<tr>
				<td class="label"><s:text name="transaction.type" />:</td>
				<td><s:select label="Transaction type" name="description.transactionType" list="#{'OUT':'Outgoing','IN':'Incoming'}" value="%{description.transactionType}" /></td>
			...
			<tr>
				...
				<td colspan="3"><s:if test="description.id==null">
					...
					<s:submit value="Commit changes" action="invitro/subculture!commit"
					...
					<s:submit value="Delete bulk" action="invitro/subculture!deleteBulk"
...
<s:if test="!readonly && description.id != null && description.status == 0">
	...
	<s:form namespace="/update" action="invitro/subculture!scanLot" method="POST">
		<table class="inputform">
			<colgroup>
				<col width="20%" />
				<col />
	...
	<s:if test="lot != null && false">
		<s:form namespace="/update" action="invitro/subculture!updateItem" method="POST">
			<table class="inputform">
				<colgroup>
					<col width="20%" />
					<col width="30%" />
			...
			<s:iterator value="description.sortedLots" status="status">
				<tr <s:if test="#readonly==false and lot.quantity - quantity < 0">class="row-error"</s:if>>
					...
					<td style="text-align: right"><s:if test="description.status==0">
						<!--<s:url id="editUrl" namespace="/update" action="invitro/subculture!edit">
							<s:param name="id" value="%{description.id}" />
							<s:param name="itemid" value="%{id}" />
						</s:url>
						<s:a href="%{editUrl}">Edit</s:a>
						-->
						<s:url id="removeUrl" namespace="/update" action="invitro/subculture!removeItem">
							<s:param name="id" value="%{description.id}" />
							<s:param name="itemid" value="%{id}" />
						</s:url>
