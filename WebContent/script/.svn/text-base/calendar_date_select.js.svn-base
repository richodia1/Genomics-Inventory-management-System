...
Date.parseFormattedString = function(string, format) { 
	return new Date.parseDate(string, format);
}
...
CalendarDateSelect.prototype = {
  ...
  dayHover: function(element) {
    ...
    this.updateFooter(hover_date.toFormattedString(this.options.get("format")));
  },
  ...
  dateString: function() {
    return (this.selection_made) ? this.selected_date.toFormattedString(this.options.get("format")) : "&#160;";
  },
  parseDate: function()
  {
...
if (!IITA) var IITA = {};
IITA.DatePicker = Class.create();

/**
 * Show a datepicker
 * @param input input field DOM control
 * @param dateFormat Date format
 */
IITA.DatePicker.show = function(input, dateFormat, defaultValue) {
	input.setAttribute('autocomplete', 'off');
	var showTime=dateFormat.indexOf('H')>=0 || dateFormat.indexOf('h')>=0;
	new CalendarDateSelect(input, {format: dateFormat, time: showTime});
}
