...
public class LotAction extends BaseAction implements Preparable {
	...
	public String readBalance() {
		...
		try {
			...
			//Integer scaleInfoId = this.selectedScales.get(ServletActionContext.getRequest().getSession().getId());
			//System.out.println("ScaleInfoId: " + scaleInfoId);
			
			BalanceScaleInfo bs = this.scaleService.find(ServletActionContext.getRequest().getSession().getId());
			
			if(bs != null){
				System.out.println("bs: " + bs);
				System.out.println("bs.getHost(): " + bs.getHost());
				System.out.println("bs.getPort(): " + bs.getPort());
				x = this.balanceService.getCurrentWeight(bs.getHost(),bs.getPort());
			}else{
				this.addFieldError("lot.weight", "Balance Scale not found/selected!");
				return Action.INPUT;
			}
